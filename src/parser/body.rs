...
use super::{identifier, parse_params, spacey, tag_identifier};
use super::{variable_identifier, Branch, Choice, Commands, Dialogue, Statements, YarnCommand};
...
pub fn statement_command(input: &str) -> IResult<&str, Statements> {
    map(spacey(yarn_commands), |command: YarnCommand| {
        Statements::Command(command)
    })(input)
}

pub fn statement_choice(input: &str) -> IResult<&str, Statements> {
    //(Statements, usize)
    ...
    let choice = Statements::ChoiceBranch(Branch {
        statements: vec![dialogue],
        ..Default::default()
    });
    Ok((input, choice))
...
pub fn yarn_conditionals(input: &str) -> IResult<&str, Vec<&str>> {
    ...
    let (input, params) =
        delimited(spacey(if_start), take_until("<<endif>>"), tag("<<endif>>"))(input)?;
...
pub fn attributes(input: &str) -> IResult<&str, (String, Vec<&str>)> {
    ...
    println!(
        "text withouth attributes {:?}",
        withouth_attributes.join(" ")
    );
...
pub fn statement_dialogue_who_what(input: &str) -> IResult<&str, Statements> {
    let (input, (who, _, what)) =
        tuple((spacey(identifier), spacey(tag(":")), alt((till_end, rest))))(input)?;
    let result = Statements::Dialogue(Dialogue {
        who: who.to_string(),
        what: what.to_string(),
        ..Default::default()
    });
...
pub fn statement_dialogue_what(input: &str) -> IResult<&str, Statements> {
    ...
    let result = Statements::Dialogue(Dialogue {
        ...
        what: what.trim_start().to_string(),
        ..Default::default()
    });
    Ok((input, result))
}
...
pub fn statement_dialogue(input: &str) -> IResult<&str, Statements> {
    let (input, result) = alt((statement_dialogue_who_what, statement_dialogue_what))(input)?;
...
pub fn get_indentation(input: &str) -> IResult<&str, usize> {
    ...
    if input.contains("->") {
        let (_, (pre_space, tag, post_space, _)) =
            tuple((space0, tag("->"), space0, not_line_ending))(input)?;
...
pub fn body(input: &str) -> IResult<&str, Branch> {
    ...
    let mut root_branch: Branch = Branch {
        statements: vec![],
        ..Default::default()
    }; // this is the root branch after the end of the parsing
                        ...
                        while let Some(mut parent_choice) = choices_stack.pop() {
                            if choices_stack.len() > 0 {
                                parent_choice
                                    ...
                                    .unwrap()
                                    .statements
                                    .push(Statements::Choice(child.clone()));
                                child = parent_choice;
                            } else {
                                parent_choice
                                    .branches
                                ...
                                root_branch
                                    .statements
                                    .push(Statements::Choice(parent_choice.clone()));
                            }
                        }
                    } else {
                        root_branch
...
fn empty_line(input: &str) -> IResult<&str, &str> {
    recognize(many_till(space0, alt((tag("\n"), eof))))(input)
}
